...
public class UserServiceTest {
    ...
    void updateUser_Success() {
        UserRequestDTO updateRequest = new UserRequestDTO(
                "Jane", "Doe", "janedoe", "newpassword123", "ADMIN"
        );
    ...
    @Test
    void searchByUsername_NoResults_ReturnsEmptyList() {
        when(userRepository.findByUsernameContainingIgnoreCase("xyz"))
                .thenReturn(List.of());

        List<UserResponseDTO> result = userService.searchByUsername("xyz");
    ...
    void filterByRole_InvalidRole_ThrowsIllegalArgumentException() {
        IllegalArgumentException ex = assertThrows(IllegalArgumentException.class,
                () -> userService.filterByRole("SUPERADMIN"));
    ...
    void searchAndFilter_UsernameOnly_Success() {
        when(userRepository.searchAndFilter("john", null))
                .thenReturn(List.of(mockUser));
    ...
    void searchAndFilter_RoleOnly_Success() {
        when(userRepository.searchAndFilter(null, User.Role.USER))
                .thenReturn(List.of(mockUser));
    ...
    void searchAndFilter_UsernameAndRole_Success() {
        when(userRepository.searchAndFilter("john", User.Role.USER))
                .thenReturn(List.of(mockUser));
    ...
    void searchAndFilter_NoResults_ReturnsEmptyList() {
        when(userRepository.searchAndFilter("xyz", null))
                .thenReturn(List.of());
    ...
    void updateUser_DuplicateUsername_ThrowsDuplicateEntryException() {
        UserRequestDTO updateRequest = new UserRequestDTO(
                "John", "Doe", "existinguser", "password123", "USER"
        );
